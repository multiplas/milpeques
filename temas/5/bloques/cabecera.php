...
<div id="top" style="min-height: 68px; z-index: 1000000">
    ...
    <?php
        echo "<div id='no-movil' style='float:right; margin-top: 0px; text-align: right;'>";
        if($Empresa['mcatalogo'] == 0){        
        if(count($cestanum)>0){
            ?>
                    <a class="cesta" href="<?=$draizp?>/<?=$_SESSION['lenguaje']?>cesta"><span class="boton"><img src="/source/cesta.png"></span> <span class="num-cesta"><?=count($cestanum)?></span></a>
            <?php
                }
                if(count($presupuestonum)>0){
            ?>
                <a class="cesta" href="<?=$draizp?>/<?=$_SESSION['lenguaje']?>presupuesto"><span id="presupuesto" class="boton">Presupuesto</span> <span class="num-cesta"><?=count($presupuestonum)?></span></a>
            <?php
                }
        }
                echo '</div>';
...
<?php } ?>
    ...
    <div id="logo" <?=$logoSup != '' ? '':'style="height:200px"'?>>
                <?php if($logoSup != ''){ ?><a href="<?=$draizp?>/<?=$_SESSION['lenguaje']?>"><img src="<?=$draizp?>/source/<?=$logoSup?>" alt="" style="width: 100%" /></a><?php } ?>
	</div>
