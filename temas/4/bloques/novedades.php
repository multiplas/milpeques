...
<style>
	...
	<div class="muestra" style="text-align: center">
		...
		?>
				<div id='producto_<?=$i?>' <?=$Empresa['ftamano'] == 1 ? '': 'style="width: 24.6%; display: inline-block;"'?>  class="<?=$Empresa['ftamano'] == 1 ? 'producto': ''?>" style="border: 1px solid rgb(221, 221, 221); padding: 15px; -webkit-animation-delay: <?=$i*0.2?>s; -moz-animation-delay: <?=$i*0.2?>s;">
					...
					<?php if($Empresa['actDes'] == 1){?><?=$productosMN[$i]['descuento'] != 0 && $productosMN[$i]['precio'] != 'Consultar' ? '<br>' : '' ?><span class="descuento"><?=$productosMN[$i]['descuento'] != 0 && $productosMN[$i]['precio'] != 'Consultar' ? '-'.$productosMN[$i]['descuento'].' '.$productosMN[$i]['descuentoe'] : ''?></span><?php } ?>
					<?php if($Empresa['actPreAnt'] == 1){?><span class="precioa"><?=$productosMN[$i]['descuento'] != 0 && $productosMN[$i]['precio'] != 'Consultar' ? number_format(ConvertirMoneda($Empresa['moneda'],$_SESSION['divisa'],$productosMN[$i]['precio_ant']), 2, ',', '.').$_SESSION['moneda'] : ''?></span><?php } ?><br>
                    <?php if($_SESSION['usr'] != null || ($_SESSION['usr'] == null && $Empresa['registro'] == 1)){ ?>
                        <?php if($Empresa['actPre'] == 1){ ?><span class="precio"><?=$productosMN[$i]['tprecio'] != '' ? $productosMN[$i]['tprecio'] : ($productosMN[$i]['precio'] != 'Consultar' ? number_format(ConvertirMoneda($Empresa['moneda'],$_SESSION['divisa'],$productosMN[$i]['precio']), 2, ',', '.') : 'Consultar')?><?=$productosMN[$i]['precio'] != 'Consultar' ? $productosMN[$i]['tprecio'] != '' ? '' : $_SESSION['moneda'] : ''?></span><?php } ?>
    ...
    <div class="muestra2" style="text-align: center;">
        ...
        if(count($productosMN) > 0){ echo "</div></div>"; } ?>		
