												...
												<?php
													foreach ($listados2 AS $listado)
													{
														...
														<tr>
															...
															<td><?=number_format(($listado['precio']+$listado['portes']), 2, ',', '.')?></td>
															<td><?=ucwords($listado['estado'])?></td>
															<td style="text-align: center;">
																<a href="?confirmarcontra=<?=$listado['id']?>" onclick="return confirm('Desea confirmar la compra por el método <?=$listado['estado']?> \'\'#<?=$listado['id']?> - <?=strtoupper($listado['secreto'])?>\'\'?');"><i style="color: green;" class="fa fa-check fa-lg"></i></a>
															</td>
														</tr>
														<?php
													}
