...
'<table style="border: solid 1px #333; padding: 15px;">
	<thead>
		<tr>
			...
			<th colspan="2" style="text-align: right"><h2>Albarán</h2></th>
		</tr>
	</thead>
        <tbody>
...
<table style="border: solid 1px #333; padding: 4px;">
	...
	foreach($_SESSION['productos'] AS $producto)
	{
		...
		'<tr style="background: '.$colorrw.';">
			<td style="border-left: solid 1px #333;"><strong>'.$producto['nombre'].'</strong> (Ref:'.$producto['referencia'].') '.($producto['talla']!=null ? '<br><small>'.$producto['fechas'].'</small>' : '').($producto['fechas']!=null ? '<br>'.$producto['fechas'] : '').($producto['extra']!=null ? '<br>'.$producto['extra'] : '').'</td>
