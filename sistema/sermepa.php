...
class Sermepa{
    ...
    public function firma()
    {
        ...
        //$mensaje = $this->_setImporte . $this->_setPedido . $this->_setFuc . $this->_setMoneda . $this->_setClave;
        if(strlen(trim($mensaje)) > 0){
            // Cálculo del SHA1    
			echo 'A: '.$mensaje.'<br>';
			$mensaje = sha1($mensaje);
			echo 'B: '.$mensaje.'<br>';
            $this->_setFirma = $mensaje;
        }
        else{
    ...
    public function create_form()
    {
		...
		$cifrada = strtoupper(sha1($this->_setImporte.$this->_setPedido.$this->_setFuc.$this->_setMoneda.$this->_setTransactionType.$this->_setUrlNotificacion.$this->_setClave));
        $formulario='
        <form action="'.$this->_setEntorno.'" method="post" id="'.$this->_setNameForm.'" name="'.$this->_setNameForm.'" >
            ...
            <input type="hidden" name="Ds_Merchant_MerchantSignature" value="'.$cifrada.'" />       
        ';
